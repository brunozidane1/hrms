...
export const atsService = {
  ...
  listCareers: (params: ListCareersParams = {}): Promise<PaginatedResponse<CareerJob>> => {
    ...
    const suffix = query.toString() ? `?${query.toString()}` : "";
    return apiClient.get<PaginatedResponse<CareerJob>>(`/api/careers${suffix}`, { auth: false });
  },

  getCareerJob: (jobId: string): Promise<CareerJob> => apiClient.get<CareerJob>(`/api/careers/${jobId}`, { auth: false }),

  applyCareer: (jobId: string, payload: CareerApplicationPayload): Promise<{ id: string; status: string; applied_at: string }> =>
    apiClient.post<{ id: string; status: string; applied_at: string }>(`/api/careers/${jobId}/apply`, payload, { auth: false }),
